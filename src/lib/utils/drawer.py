...
class Drawer(object):
    ...
    def draw_dets_video(self, frame, ret, total_time, reid, dist = None, best_index = None, query_len = None, color=None, label=''):
        ...
        frameDC = copy.deepcopy(frame)
        self._put_label(frameDC, label)
        total_text = 'Total Time per Frame: {:0.2f} ms'.format(total_time * 1000)
        for i in range(len(ret['results'])):
            #_color = self.color_palette[i%8]
            l, t, r, b = ret['results'][i]['bbox']
            
            cv2.rectangle(frameDC, (int(l), int(t)), (int(r), int(b)), self.color_palette[0], 2)

            if reid:
                if len(dist.shape) == 1:
                    for index in best_index:
                        if i == index and dist[index] <= 0.75*query_len:
                            l, t, r, b = ret['results'][index]['bbox']
                            r = r + 5
                            b = b + 5
                            l = l - 5
                            t = t - 5
                
                            cv2.rectangle(frameDC, (int(l), int(t)), (int(r), int(b)), self.color_palette[5], 3)
                else:
                    if dist.shape[1] == 1:
                        for index in best_index:
                            if i == index and dist[index][0] <= 0.75*query_len:
                                l, t, r, b = ret['results'][index]['bbox']
                                r = r + 5
                                b = b + 5
                                l = l - 5
                        ...
                        for index in best_index:
                            for j in range(len(dist[index])):
                                if i == index and dist[index][j] <= 0.75*query_len:
                                    l, t, r, b = ret['results'][index]['bbox']
                                    r = r + 5
                                    b = b + 5
                                    l = l - 5
            ...
            if str(ret['results'][i]['tracking_id']) is not None:
                text_size = cv2.getTextSize(
                    str(ret['results'][i]['tracking_id']), cv2.FONT_HERSHEY_PLAIN, 1, 1)

                center = l + 5, t + 5 + text_size[0][1]
                x, y = center
                center = int(x), int(y)
                pt2 = int(l + 10 + text_size[0][0]), int(t + 10 + text_size[0][1])

                cv2.rectangle(frameDC, (int(l), int(t)), pt2, self.color_palette[1], -1)
                cv2.putText(frameDC, str(ret['results'][i]['tracking_id']), center, cv2.FONT_HERSHEY_PLAIN,
                            1, (255, 255, 255), 1)

            #cv2.putText(frameDC, text, (l+5, b-10), self.font, self.indFontScale, _color, self.fontThickness)    
        cv2.putText(frameDC, total_text, (10, 30), self.font, self.fontScale * 0.75, (0, 0, 255), self.indFontThickness) 
        return frameDC
