...
def train_joint(args, #train_vric_dataloader, val_vric_dataloader,
                train_veri_dataloader, val_veri_dataloader,
    ...
    epoch_size = len(train_veri_dataloader)#train_veri_dataloader, len(train_compcars_dataloader), len(train_boxcars_dataloader), len(train_aic_dataloader))
    ...
    for e in range(args.n_epochs):
        ...
        if e % args.save_every_n_epoch == 0:
            ...
            torch.save(base_net.state_dict(),os.path.join(args.save_model_dir,'model_%d_base.pth'%(e)))
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--vric_txt', required=True, help='txt for VeRi dataset')
    ...
    # Load dataset
    #vric_dataset = load_dataset(args.vric_txt)
    ...
    # vric_dataset = TripletImage_Dataset(args.vric_txt, crop=args.crop, flip=args.flip, jitter=args.jitter, 
    ...
    # train_vric_dataloader = Get_train_DataLoader(vric_dataset, batch_size=args.class_in_batch)
    # val_vric_dataloader = Get_val_DataLoader(vric_dataset, batch_size=args.class_in_batch)
    ...
    # vric_id_net = models.NLayersFC(base_net.output_dim, vric_dataset.n_id)
    ...
    # Train
    train_joint(args, #train_vric_dataloader, val_vric_dataloader, \
                train_veri_dataloader, val_veri_dataloader, \
                # train_compcars_dataloader, val_compcars_dataloader, \
                # train_boxcars_dataloader, val_boxcars_dataloader, \
                # train_aic_dataloader, \
                # test_compcars_dataloader,\
                # test_boxcars_dataloader,\
                base_net, veri_id_net)#, veri_id_net, color_net, compcars_model_net, boxcars_model_net)
